...
/**
 * Customer password recovery (`/[lang]/area/recupera-password`). Two states on one
 * page, switched by `?token=`: forgot (request email) / reset (set new password).
 * Reuses the shared PasswordRecoveryForm, pointing "back to sign in" at the
 * customer login. This is also how a guest-booking shell account (ADR-0044) sets
 * its first password to claim the account.
 */
export default async function CustomerPasswordRecoveryPage({
  params,
  searchParams,
}: {
  params: Promise<Params>;
          ...
          <div className="w-full max-w-[420px] rounded-[15px] border border-soft-grey bg-white p-6 sm:p-8">
            ...
            <div className="mt-6">
              <PasswordRecoveryForm
                ...
                backToLoginHref={`/${lang}/area/accedi`}
              />
            </div>
          </div>
        </Container>
